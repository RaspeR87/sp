...
if (spField == null)
{
	throw new SPException(SPResource.GetString(Strings.FieldGone));
}
if (spList == null)
	Version = spField.Version;
%>
<%
if (null == spList)
{
...
<asp:Content ContentPlaceHolderId="PlaceHolderPageDescription" runat="server">
	<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_usepagetoeditquestion%>" EncodeMethod='HtmlEncode'/>
</asp:Content>
<asp:Content ContentPlaceHolderId="PlaceHolderPageTitle" runat="server">
	<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_editquestion%>" EncodeMethod='HtmlEncode'/>
</asp:Content>
...
strName          = spField.InternalName;
strDisplayName   = spField.Title;
bRequired        = spField.Required;
bEnforceUniqueValues = spField.EnforceUniqueValues;
bIndexed = spField.Indexed;
bSealed          = spField.Sealed;
bCanBeDeleted   = spField.CanBeDeleted;
strDescription   = spField.Description;
FieldTypeName    = spField.TypeAsString;
strDirection     = spField.Direction;
strFromBaseType  = spField.FromBaseType?"TRUE":"FALSE";
strDisplaySize   = spField.DisplaySize;
strIMEMode       = spField.IMEMode != null ? spField.IMEMode : "";
VldFormula.Text  = spField.ValidationFormula;
VldMessage.Text  = spField.ValidationMessage;
if (spField is SPFieldMultiLineText)
{
	bUnlimitedLengthInDocumentLibrary =
		((SPFieldMultiLineText)spField).UnlimitedLengthInDocumentLibrary;
}
if (strListId == null)
{
...
if (spField.TypeAsString == FieldTypeName ||
	(spField.Type == SPFieldType.MultiChoice && FieldTypeName == "Choice"))
{
	strDefaultFormula = spField.DefaultFormula;
	fDefaultFormulaEmpty = String.IsNullOrEmpty(strDefaultFormula);
	if (fDefaultFormulaEmpty)
	{
		strDefault       = spField.DefaultValue;
	}
	switch(spField.Type)
	{
		case SPFieldType.Text:
		{
			 SPFieldText field = (SPFieldText)spField;
			iMaxLength = field.MaxLength;
			break;
		}
		case SPFieldType.Calculated:
		{
			 SPFieldCalculated field= (SPFieldCalculated)spField;
			strFormula     = field.Formula;
			CurrencyLocaleId = (int)field.Currency;
			strDecimals = Convert.ToString((int)field.DisplayFormat);
			strResultType = Convert.ToString(field.OutputType);
			bPercentage = field.ShowAsPercentage;
			strFormat    = Convert.ToString(field.DateFormat);
			break;
		}
		case SPFieldType.Number:
		{
			 SPFieldNumber field = (SPFieldNumber)spField;
			strDecimals = Convert.ToString((int)field.DisplayFormat);
			dblMin = field.MinimumValue;
			dblMax = field.MaximumValue;
			bPercentage = field.ShowAsPercentage;
			break;
		}
		case SPFieldType.Currency:
		{
			 SPFieldCurrency field = (SPFieldCurrency)spField;
			strDecimals = Convert.ToString((int)field.DisplayFormat);
			dblMin = field.MinimumValue;
			dblMax = field.MaximumValue;
			CurrencyLocaleId = (int)field.Currency;
		...
		{
			 SPFieldMultiLineText field = (SPFieldMultiLineText)spField;
			iNumLines = field.NumberOfLines;
			bAllowHyperlink = field.AllowHyperlink;
			break;
		}
		case SPFieldType.DateTime:
		{
			 SPFieldDateTime field = (SPFieldDateTime)spField;
			iCalType     = (int)field.CalendarType;
			if (iCalType > 0)
				bUseDefault = false;
			else if (iCalType == 0)
				iCalType = iWebCalType;
			if ((iCalType != 1)
				&& !String.IsNullOrEmpty(strDefault)
				&& IsPreviewableCalType(iCalType))
			{
				try
				{
					strDefault = SPUtility.GregorianISOToIntlISODate(Web, strDefault, iCalType);
				}
				catch
				{
					strDefault = "";
			...
			strFormat    = Convert.ToString(field.DisplayFormat);
			strFriendlyFormat = Convert.ToString(field.FriendlyDisplayFormat);
			if (strFriendlyFormat != "Disabled" && strFriendlyFormat != "Relative")
			{
				strFriendlyFormat = "Disabled";
			}
			break;
		...
		case SPFieldType.Choice:
		{
			...
			strFormat = Convert.ToString(field.EditFormat);
			strFillInChoice = field.FillInChoice?"TRUE":"FALSE";
			...
			strChoiceTextArea = str.ToString();
			if (field.Mappings != null)
				strHiddenSchema += field.Mappings;
			break;
		}
		case SPFieldType.MultiChoice:
		{
			 SPFieldMultiChoice field = (SPFieldMultiChoice)spField;
			strFillInChoice = field.FillInChoice?"TRUE":"FALSE";
			StringBuilder str = new StringBuilder();
			StringCollection choices = field.Choices;
			foreach(String choice in choices)
		...
		{
			SPFieldChoice field = (OutcomeChoiceField)spField;
			strFormat = Convert.ToString(field.EditFormat);
			StringBuilder str = new StringBuilder();
			StringCollection choices = field.Choices;
			foreach(String choice in choices)
		...
		{
			SPFieldRatingScale field = (SPFieldRatingScale)spField;
			StringBuilder str = new StringBuilder();
			StringCollection choices = field.Choices;
			foreach(String choice in choices)
			{
			...
			strGridChoiceTextArea = str.ToString();
			FieldTypeName = "GridChoice";
			GridEndNum = Convert.ToInt32(field.GridEndNumber);
			strGridTxtRng1   = field.GridTextRangeLow;
			strGridTxtRng2   = field.GridTextRangeAverage;
			strGridTxtRng3   = field.GridTextRangeHigh;
			strGridNATxt     = field.GridNAOptionText;
			if (field.Mappings != null)
				strHiddenSchema += field.Mappings;
			break;
		}
		...
		{
			SPFieldPageSeparator field = (SPFieldPageSeparator)spField;
			break;
		}
		case SPFieldType.Lookup:
		{
			 SPFieldLookup field = (SPFieldLookup)spField;
			strLookupList     = field.LookupList;
			if(spList != null &&
				(String.IsNullOrEmpty(strLookupList) ||
				strLookupList == "self"))
			{
			   strLookupList = spList.ID.ToString("B").ToUpper();
			}
			strLookupField   = field.LookupField;
			if (String.IsNullOrEmpty(strLookupField))
				strLookupField = "Title";
			strShowPresence = "FALSE";
			break;
		}
		case SPFieldType.User:
		{
			SPFieldUser field = (SPFieldUser)spField;
			strLookupList     = "UserInfo";
			strLookupField     = field.LookupField;
			if (String.IsNullOrEmpty(strLookupField))
				strLookupField = "Title";
			strShowPresence = field.Presence ? "" : "FALSE";
			break;
		}
		case SPFieldType.URL:
		{
			 SPFieldUrl field = (SPFieldUrl)spField;
			strFormat     = Convert.ToString(field.DisplayFormat);
			break;
		}
	}
...
if (spField is SPFieldMultiLineText)
{
	bRichText = ((SPFieldMultiLineText)spField).RichText;
	bAppendOnly = ((SPFieldMultiLineText)spField).AppendOnly;
	RichTextMode = ((SPFieldMultiLineText)spField).RichTextMode;
	if (bRichText &&
		(RichTextMode == SPRichTextMode.HtmlAsXml ||
		RichTextMode == SPRichTextMode.ThemeHtml))
	{
		bDisableRichTextModeControl = true;
	}
}
bAllowFieldTypeChange = false;
if (!spField.FromBaseType && !spField.Sealed &&
	(spField.Type == SPFieldType.Text ||
	spField.Type == SPFieldType.Choice ||
	(spField.Type == SPFieldType.Note && bRichText == false) ||
	spField.Type == SPFieldType.MultiChoice ||
	spField.Type == SPFieldType.Number ||
	spField.Type == SPFieldType.Currency ||
	spField.Type == SPFieldType.DateTime ||
	spField.Type == SPFieldType.Boolean))
{
	bAllowFieldTypeChange = true;
}
...
if (FieldTypeName == "Choice")
{
		bool distinctSuccess = false;
		if(spField.Type == SPFieldType.Text)
		{
			object[,] fieldValuesArr = null;
			uint fieldValueCount = 0;
			if (spList != null)
				fieldValueCount = spList.GetDistinctFieldValues(spField, out fieldValuesArr);
			if(fieldValueCount != 0)
			{
				for(uint i = 0; i < fieldValueCount; i++)
				{
					strChoiceTextArea += fieldValuesArr[0,i].ToString() + "\r\n";
				}
				strDefault = fieldValuesArr[0,0].ToString();
				distinctSuccess = true;
			}
		}
		if(!distinctSuccess)
		{
			String L_strDefaultChoice_Text= (string)(this.GetGlobalResourceObject("wss", "fldedit_L_strDefaultChoice_Text"));
			strChoiceTextArea = L_strDefaultChoice_Text
						+ "\r\n" + (string)(this.GetGlobalResourceObject("wss", "fldedit_L_strChoice2_Text"))
						+ "\r\n" + (string)(this.GetGlobalResourceObject("wss", "fldedit_L_strChoice3_Text"));
			strDefault = L_strDefaultChoice_Text;
		}
	}
}
%>
...
function BodyOnLoad()
{
	...
	<% if (FieldTypeName == "GridChoice") {%>
		showNATxt();
	<%}%>
	 <% if (spField.TypeAsString != FieldTypeName &&
			!(spField.Type == SPFieldType.MultiChoice && FieldTypeName == "Choice"))
	 { %>
		resetBranchingOptions();
	 <% }
		else if (spField.Type == SPFieldType.Choice) {%>
		showFillinSection();
	<%}%>
	<% if (spField.Type == SPFieldType.MultiChoice) { %>
		disableUniquenessIfMultiChoice();
	<%}%>
...
<% if (spField.Type == SPFieldType.Choice)
	{%>
	if (GetRadioButtonValue(document.forms[MSOWebPartPageFormName].FillInChoice) == "TRUE")
		document.getElementById("BranchingFillinSection").style.display = "";
	else
		document.getElementById("BranchingFillinSection").style.display = "none";
	<%}%>
}
...
%>
	...
	function SubmitPage(fAddMore, btn) {
		...
		var pageCommand = "Cmd=UpdateField";
...
<%      }
		...
		var FromBaseType= frm.FromBaseType.value;
		var DisplaySize = frm.DisplaySize.value;
<%
if (bAllowFieldTypeChange)
{
%>
...
%>
			...
			<%
			if (spField.Type == SPFieldType.Note &&
				(spField as SPFieldMultiLineText).RichText)
			{
				if ((spField as SPFieldMultiLineText).RichTextMode ==
						SPRichTextMode.FullHtml)
				{
					...
					if (Type == "Note" &&
						...
						RichTextMode != "FullHtml")
					{
						var L_loseFullFormatting_Text = "<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_dataloss_enhancedtorichtext%>' EncodeMethod='EcmaScriptStringLiteralEncode'/>";
						if (!confirm(L_loseFullFormatting_Text))
						{
							return false;
						}
					}
				...
				if (Type =="Note" && RichText != "TRUE")
				{
					...
					{
						%>
						var L_loseAllFormatting_Text = "<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_dataloss_enhancedtoplain%>' EncodeMethod='EcmaScriptStringLiteralEncode'/>";
						var loseFormatting = L_loseAllFormatting_Text;
						<%
					}
					else
					{
						%>
						var L_loseFormatting_Text = "<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_dataloss_richtexttoplain%>' EncodeMethod='EcmaScriptStringLiteralEncode'/>";
						var loseFormatting = L_loseFormatting_Text;
						<%
					}
					%>
					if (!confirm(loseFormatting))
					{
						return false;
					}
				}
			...
			%>
			var fromType = <%SPHttpUtility.WriteAddQuote(SPHttpUtility.EcmaScriptStringLiteralEncode(spField.TypeAsString),this.Page);%>;
			<%
				if (spList == null)
				{
			%>
				var strDataLoss = "<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_dataloss_template%>' EncodeMethod='EcmaScriptStringLiteralEncode'/>";
			<%
				}
				else
				{
			%>
				var strDataLoss = "<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_dataloss%>' EncodeMethod='EcmaScriptStringLiteralEncode'/>";
			<%
				}
			%>
			if (fDataLoseConvert(fromType, Type))
			{
				var strCANTCNVT_TEXT= StBuildParam(strDataLoss, GetTypeDesc(fromType), GetTypeDesc(Type));
				if (!confirm(strCANTCNVT_TEXT))
				{
					return false;
				}
			}
	var JumpTo = frm.JumpTo   ? TrimSpaces( GetSelectedValue(frm.JumpTo) ): "";
	var JumpToYes = frm.JumpToYes   ? TrimSpaces( GetSelectedValue(frm.JumpToYes) ): "";
	var JumpToNo = frm.JumpToNo   ? TrimSpaces( GetSelectedValue(frm.JumpToNo) ): "";
	var JumpToFillin = frm.JumpToFillin   ? TrimSpaces( GetSelectedValue(frm.JumpToFillin) ): "";
			var Schema = ('<Field ' +
				...
				(JumpTo       ? 'JumpTo="'      + JumpTo                         + '" ' : '') +
				(JumpToYes    ? 'JumpToYes="'   + JumpToYes                      + '" ' : '') +
				(JumpToNo     ? 'JumpToNo="'    + JumpToNo                       + '" ' : '') +
				(JumpToFillin ? 'JumpToFillinChoice="'+ JumpToFillin             + '" ' : '') +
<% if (bShowIME)
   {
%>
			...
			if(Type == "Choice" || Type == "MultiChoice" || Type == "OutcomeChoice")
			{
				...
				for( i = 0; i < ChoicesArr.length; i ++ )
				{
					...
					if(Choice) {
						...
						var JumpToSelectName = 'JumpTo' + i;
						var JumpTo = frm[JumpToSelectName] ? TrimSpaces( GetSelectedValue(frm[JumpToSelectName]) ): "";
						Schema += '<CHOICE ' +
									(JumpTo? 'JumpTo="' + JumpTo + '" ' : '') +
									'>' + SimpleHTMLEncode(Choice) + '</CHOICE>';
					}
				}
				Schema += '</CHOICES>';
...
<%
if (Web.UICulture.LCID == System.Globalization.CultureInfo.CurrentUICulture.LCID)
{
...
<SharePoint:ScriptBlock runat="server" WriteEnabled="true">
	function doesFieldNameConflict( strName )
	{
		if (g_FieldName[strName.toLowerCase()] &&
			 strName.toLowerCase() != <%SPHttpUtility.WriteAddQuote(SPHttpUtility.EcmaScriptStringLiteralEncode(spField.Title),this.Page);%>.toLowerCase())
			return true;
		else
			return false;
	}
	...
	function resetBranchingOptions()
	{
		var frm = document.forms[MSOWebPartPageFormName];
		document.getElementById("BranchingReset").style.display = '';
		document.getElementById("BranchingSection").style.display = 'none';
<%
	switch (spField.Type)
	{
	case SPFieldType.Choice:
...
%>
		i = 0;
		JumpToDropDownName = 'JumpTo' + i;
		while (frm[JumpToDropDownName] != null)
		{
			frm[JumpToDropDownName].selectedIndex = 0;
			i++;
			JumpToDropDownName = 'JumpTo' + i;
		}
<%
	...
	{
%>
		frm["JumpToYes"].selectedIndex = 0;
		frm["JumpToNo"].selectedIndex = 0;
<%
		break;
	}
	default:
...
<% if (spField.Type == SPFieldType.Choice ||
	...
	{
%>
	function OnChoiceFormatClick()
	{
		var frm = document.forms[MSOWebPartPageFormName];
		var choiceFormat = GetRadioButtonValue(frm.Format);
<%      if (spField.Type == SPFieldType.Choice)
		{
%>
			if ( choiceFormat == "Checkboxes")
			{
				resetBranchingOptions();
			}
<%
...
<input type="hidden" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strFromBaseType),this.Page);%> name="FromBaseType" />
<input type="hidden" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strDisplaySize),this.Page);%> name="DisplaySize" />
<!-- Name and Type-->
...
%>
					...
					<tr>
						<td>
							...
							<table border="0" cellspacing="1">
								<tr>
									<td colspan="2">
											<textarea class="ms-long" cols=40 rows=5 name="DisplayName" id="idColName" size=30 style="display:none;"><%SPHttpUtility.WriteHtmlEncode(strDisplayName,this.Page);%></textarea>
		...
		<SharePoint:ScriptBlock runat="server" WriteEnabled="true">
			(function() {
				var currDisplayName = $("#idColName").val().split("||");
				
				if (currDisplayName.length > 0) {
					$("#idColNameSI").val(currDisplayName[0].trim());
				}

				if (currDisplayName.length === 1) {
					$("#idColNameEN").val(currDisplayName[0].trim());
				} else if (currDisplayName.length > 1) {
					$("#idColNameEN").val(currDisplayName[1].trim());
				}
			})();
				...
				<table border="0" cellspacing="4">
					<tr>
						<td class="ms-authoringcontrols">
							<%SPHttpUtility.WriteHtmlEncode(spField.TypeShortDescription, this);%>
							<input type="hidden" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(FieldTypeName),this.Page);%> name="Type" />
						</td>
					</tr>
				</table>
<%
...
%>
				<table border="0" cellspacing="2">
					<tr>
						<td width="1"><img src="/_layouts/15/images/blank.gif?rev=40" width='1' height='1' alt="" data-accessibility-nocheck="true"/></td>
						<td>
							<table width="100%" cellpadding="0" cellspacing="1" border="0">
<%
							if (spField.Type == SPFieldType.Text ||
								spField.Type == SPFieldType.Choice ||
								spField.Type == SPFieldType.Note ||
								spField.Type == SPFieldType.MultiChoice ||
								spField.Type == SPFieldType.Number ||
								spField.Type == SPFieldType.Currency ||
								spField.Type == SPFieldType.DateTime ||
	...
	<tr>
		...
		<td class="ms-authoringcontrols" id="onetidTypeRichText"><label for="onetidRichText"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_RichTextID%>" EncodeMethod='HtmlEncode'/></label><font size="3">&#160;</font><br />
			<table border="0" cellspacing="1">
				...
				<tr>
					<td class="ms-authoringcontrols">
					<% if ( (bRichText && RichTextMode == SPRichTextMode.Compatible) == true ) { %> <input type="radio" name="RichText" value="COMPATIBLE" id="onetidRichTextCompatible" <% if (bDisableRichTextModeControl) { %>disabled="true"<% } %>title="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_richtext_compatible%>' EncodeMethod='HtmlEncode'/>" checked="checked" /> <% ; } else { %> <input type="radio" name="RichText" value="COMPATIBLE" id="onetidRichTextCompatible" <% if (bDisableRichTextModeControl) { %>disabled="true"<% } %>title="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_richtext_compatible%>' EncodeMethod='HtmlEncode'/>" /> <% ; } %>
					</td>
					<td class="ms-authoringcontrols" id="onetidNo"><label for="onetidRichTextCompatible"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_richtext_compatible%>" EncodeMethod='HtmlEncode'/></label></td>
				</tr>
				<tr>
					<td class="ms-authoringcontrols">
...
%>
	...
	<tr>
		...
		<td>
			<input type="hidden" name="Default" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strDefault),this.Page);%>" />
			<input type="hidden" name="DefaultURLDesc" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strDefaultURLDesc),this.Page);%> />
		</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		...
		<td class="ms-authoringcontrols" id="onetidLookupList"><label for="idLookupList"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_getinfofrom%>" EncodeMethod='HtmlEncode'/></label><font size="3">&#160;</font><br />
			<table border="0" cellspacing="1">
				<tr>
					<td class="ms-authoringcontrols">
						<input type="hidden" name="LookupList" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strLookupList),this.Page);%> />
<%
...
<%
						...
						else if (strLookupList != "Docs")
						{
							...
							try
							{
								...
								{
									SPHttpUtility.WriteHtmlEncode(spListLookup.Title,this.Page);
								}
							}
	...
	<tr><td colspan="2">&#160;</td>
		...
		<td class="ms-authoringcontrols" id="onetidLookupListField"><label for="idLookupField"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_inthiscolumn%>" EncodeMethod='HtmlEncode'/></label><font size="3">&#160;</font><br />
			<table border="0" cellspacing="1">
				<tr>
					<td class="ms-authoringcontrols">
						<input type="hidden" name="LookupField" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strLookupField),this.Page);%> />
			<%
				if (strLookupList == "UserInfo")
				{
					...
					{
			%>
						<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_fldgutsID%>" EncodeMethod='HtmlEncode'/><!-- -->
			<%
					}
					else if (strLookupField == "EMail")
					{
			%>
						<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_fldgutsEMail%>" EncodeMethod='HtmlEncode'/><!-- -->
			<%
					}
					else if (strLookupField == "Name")
					{
			%>
						<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_fldgutsLogin%>" EncodeMethod='HtmlEncode'/><!-- -->
			<%
					}
					else
					{
			%>
						<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_fldgutsFullName%>" EncodeMethod='HtmlEncode'/><!-- -->
			<%
					}
				}
				else if (strLookupList != "Docs")
						...
						try
						{
							...
							switch (spField.Type)
							{
								...
								default:
									...
									SPHttpUtility.WriteHtmlEncode(L_InvalidLookupFieldType_Text,this.Page);
								break;
							}
						}
						catch (ArgumentException ae)
						{
							SPHttpUtility.WriteHtmlEncode(strLookupField,this.Page);
							string L_DeletedLookupField_Text =
								(string)(this.GetGlobalResourceObject("wss", "fldedit_L_DeletedLookupField_Text"));
							SPHttpUtility.WriteHtmlEncode(L_DeletedLookupField_Text,this.Page);
						}
					}
				}
			%>
...
%>
				...
				<tr>
					<td>
						...
						<table border="0" cellspacing="1">
							<tr>
								<td>
									<textarea class="ms-input" name="Choices" id="idChoices" rows="4" cols="40" wrap="off"  onchange="clearOutDefaultWithCheck()" style="display:none;"><%SPHttpUtility.WriteHtmlEncode(strChoiceTextArea,this.Page);%></textarea>
									<textarea class="ms-input" name="ChoicesSI" id="idChoicesSI" rows="4" cols="40" wrap="off"  onchange="clearOutDefaultWithCheck()" onKeyUp="CopyAnswers();"></textarea>
								</td>
							</tr>
						</table>
					</td>
				...
				<tr>
					<td>
						...
						<table border="0" cellspacing="1">
							<tr>
								<td>
									<textarea class="ms-input" name="ChoicesEN" id="idChoicesEN" rows="4" cols="40" wrap="off"  onchange="clearOutDefaultWithCheck()" onKeyUp="CopyAnswers();"></textarea>
								</td>
							</tr>
						</table>
					</td>
	...
	<SharePoint:ScriptBlock runat="server" WriteEnabled="true">
		(function() {
			...
			var currChoicesList = $("#idChoices").val().split("\n");
			for (var i=0; i < currChoicesList.length; i++) {
				var currChoices = currChoicesList[i].split("||");

				if (currChoices.length > 0) {
					$("#idChoicesSI").val($("#idChoicesSI").val() + currChoices[0].trim() + "\n");
				}

				if (currChoices.length === 1) {
					$("#idChoicesEN").val($("#idChoicesEN").val() + currChoices[0].trim() + "\n");
				}
				if (currChoices.length > 1) {
					$("#idChoicesEN").val($("#idChoicesEN").val() + currChoices[1].trim() + "\n");
				}
			}
		})();
			...
			<table border="0" cellspacing="1">
				<%
					...
					if (spField.TypeAsString == FieldTypeName ||
							(spField.Type == SPFieldType.MultiChoice && FieldTypeName == "Choice"))
					{
					strChoiceFormatOnClick = "onclick = 'OnChoiceFormatClick(); disableUniquenessIfMultiChoice(); showValidationSection(); return true;'";
					}
				%>
				<tr>
					<td class="ms-authoringcontrols">
		...
		<td class="ms-authoringcontrols" id="onetidDefaultChoiceValue">
			...
			<table border="0" cellspacing="1">
				<tr>
					<td colspan="2"></td>
					<td>
						<input class="ms-input" type="text" name="DefaultText" value=<% if (fDefaultFormulaEmpty) SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strDefault),this.Page);else SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strDefaultFormula),this.Page); %> id="onetidIODefChoiceValue" title="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_defaultvalue%>' EncodeMethod='HtmlEncode'/>" />
					</td>
				</tr>
			</table>
		</td>
...
%>
	...
	<tr>
		...
		<td class="ms-authoringcontrols" id="onetidEnterGChoice"><label for="onetidEnterGChoice123"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_typeeachsubquestiononseparate%>" EncodeMethod='HtmlEncode'/>:</label><font size="3">&#160;</font><br />
			<table border="0" cellspacing="1">
				<tr>
					<td>
						<textarea name="GridChoices" class="ms-long" rows="4" cols="40" wrap="off" id="onetidEnterGChoice123"><%SPHttpUtility.WriteHtmlEncode(strGridChoiceTextArea,this.Page);%></textarea>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		...
		<td class="ms-authoringcontrols" id="onetidGridChoiceValue"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_onetidGridChoiceValue2%>" EncodeMethod='HtmlEncode'/><font size="3">&#160;</font><br />
			<table border="0" cellspacing="1">
				<tr>
					<td>
						<input type="text" title="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_L_GridLow_Text%>' EncodeMethod='HtmlEncode'/>" size="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_idAttrSizeLowAvgHigh%>' EncodeMethod='HtmlEncode'/>" name="GridTxtRng1" id="onetidIDGridTxtRng1" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strGridTxtRng1),this.Page);%> />&#160;
						<input type="text" title="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_L_GridAverage_Text%>' EncodeMethod='HtmlEncode'/>" size="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_idAttrSizeLowAvgHigh%>' EncodeMethod='HtmlEncode'/>" name="GridTxtRng2" id="onetidIDGridTxtRng2" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strGridTxtRng2),this.Page);%> />&#160;
						<input type="text" title="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_L_GridHigh_Text%>' EncodeMethod='HtmlEncode'/>" size="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_idAttrSizeLowAvgHigh%>' EncodeMethod='HtmlEncode'/>" name="GridTxtRng3" id="onetidIDGridTxtRng3" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strGridTxtRng3),this.Page);%> />&#160;
					</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		...
		<SharePoint:ScriptBlock runat="server" WriteEnabled="true">
			...
			&#160; &#160;
			<input type="checkbox" onclick="showNATxt();" name="GridShowNA" id="onetidShowNA" value="on" <% if (strGridNATxt != null && strGridNATxt != "") { %> checked <% } %> /> &#160; &#160;
			<div id="GridOptionBlock"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_naoptiontext%>" EncodeMethod='HtmlEncode'/>&#160;
			<% if (strGridNATxt != null && strGridNATxt != "") {%>
			<input type="text"
title="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_naoptiontext_title%>' EncodeMethod='HtmlEncode'/>" size="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_naooptiontext_size%>' EncodeMethod='HtmlEncode'/>" name="GridNATxt" id="onetidIDGridNATxt" value=<%SPHttpUtility.WriteAddQuote(SPHttpUtility.HtmlEncode(strGridNATxt),this.Page);%> />
			<%} else { %>
			<input type="text"
...
<!-- Push Down Settings -->
	...
	<tr>
		...
		<td class="ms-descriptiontext ms-formdescriptioncolumn-wide" valign="top">
			<table border="0" cellpadding="1" cellspacing="0">
				<tr><td height="28" valign="top" class="ms-sectionheader"><h3 class="ms-standardheader"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_pushdown_header%>" EncodeMethod='HtmlEncode'/></h3></td></tr>
				<tr><td class="ms-propertysheet"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_pushdown_description%>" EncodeMethod='HtmlEncode'/></td></tr>
			</table>
		</td>
		<td class="ms-authoringcontrols" width="10">&#160;</td>
		<td class="ms-authoringcontrols">
			<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_pushdown_instruction%>" EncodeMethod='HtmlEncode'/><br />
			<table border="0" cellpadding="1" cellspacing="0">
			<tr>
...
<!--Branching settings -->
	...
	<tr>
		...
		<td class="ms-authoringcontrols" id="onetidBranching">
		  <span id="BranchingReset" style="display:none">
		  <SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,survey_branching_reset%>" EncodeMethod='HtmlEncode'/>
		  </span>
		  <span id="BranchingSection">
		  <table id="onetidBranchingTable" class="ms-authoringcontrols" border="0" width="100%" cellspacing="0" cellpadding="2" >
			<tr>
			<th scope="col" class="ms-authoringcontrols" align="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,multipages_direction_left_align_value%>' EncodeMethod='HtmlEncode'/>">
				<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,survey_branching_possible_choices%>" EncodeMethod='HtmlEncode'/>
			</th>
			<th scope="col" class="ms-authoringcontrols" align="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,multipages_direction_left_align_value%>' EncodeMethod='HtmlEncode'/>">
				<SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,survey_branching_jump_to%>" EncodeMethod='HtmlEncode'/>
			</th>
			</tr>
<%
	switch (spField.Type)
...
%>
			<tr>
			<td class="ms-authoringcontrols"><label for="<%="onetidchoicebranch"+iChoiceIndex.ToString()%>"><%SPHttpUtility.WriteHtmlEncode(choice,this.Page);%></label></td>
			<td class="ms-authoringcontrols">
			<select name=JumpTo<%SPHttpUtility.WriteNoEncode(iChoiceIndex,this.Page);%> id="<%="onetidchoicebranch"+iChoiceIndex.ToString()%>">
				<option value=""><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,SurveyBranchNoBranching%>" EncodeMethod='HtmlEncode'/></option>
<%
				for (int i=indexOfCurrentField+1; i<spFields.Count; i++)
				{
					SPField spFieldTmp = spFields[i];
					if (BranchingAllowedTo(spFieldTmp))
					{
%>
	<option <%SPHttpUtility.WriteNoEncode((field.ChoicesJumpTo[iChoiceIndex] == spFieldTmp.InternalName)?"selected":"",this.Page);%> Value=<%SPHttpUtility.WriteHtmlEncode(spFieldTmp.InternalName,this.Page);%>><%SPHttpUtility.WriteHtmlEncode(TruncatedTitle(spFieldTmp.Title),this.Page);%></option>
<%
					}
				}
%>
...
%>
			<tbody id="BranchingFillinSection">
			<tr>
			<td class="ms-authoringcontrols"><label for="<%="onetidchoicebranch"+iChoiceIndex.ToString()%>"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,fldedit_fillinjumpto%>" EncodeMethod='HtmlEncode'/></label></td>
			<td class="ms-authoringcontrols">
			<select name="JumpToFillin" id="<%="onetidchoicebranch"+iChoiceIndex.ToString()%>">
				<option value=""><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,SurveyBranchNoBranching%>" EncodeMethod='HtmlEncode'/></option>
<%
			for (int i=indexOfCurrentField+1; i<spFields.Count; i++)
			{
				...
				{
		%>
			<option <%SPHttpUtility.WriteNoEncode((field.FillinChoiceJumpTo == spFieldTmp.InternalName)?"selected":"",this.Page);%> Value=<%SPHttpUtility.WriteHtmlEncode(spFieldTmp.InternalName,this.Page);%>><%SPHttpUtility.WriteHtmlEncode(TruncatedTitle(spFieldTmp.Title),this.Page);%></option>
		<%
				}
			}
%>
...
%>
			<tr>
			<td class="ms-authoringcontrols"><label for="onetidboolyesbranch"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,Yes%>" EncodeMethod='HtmlEncode'/></label></td>
			<td class="ms-authoringcontrols">
			<select name="JumpToYes" id="onetidboolyesbranch">
				<option value=""><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,SurveyBranchNoBranching%>" EncodeMethod='HtmlEncode'/></option>
<%
			for (int i=indexOfCurrentField+1; i<spFields.Count; i++)
			{
				...
				{
		%>
			<option <%SPHttpUtility.WriteNoEncode((field.JumpToYesField == spFieldTmp.InternalName)?"selected":"",this.Page);%> Value=<%SPHttpUtility.WriteHtmlEncode(spFieldTmp.InternalName,this.Page);%>><%SPHttpUtility.WriteHtmlEncode(TruncatedTitle(spFieldTmp.Title),this.Page);%></option>
		<%
				}
			}
%>
			...
			<tr>
			<td class="ms-authoringcontrols"><label for="onetidboolnobranch"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,No%>" EncodeMethod='HtmlEncode'/></label></td>
			<td class="ms-authoringcontrols">
			<select name="JumpToNo" id="onetidboolnobranch">
				<option value=""><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,SurveyBranchNoBranching%>" EncodeMethod='HtmlEncode'/></option>
<%
			for (int i=indexOfCurrentField+1; i<spFields.Count; i++)
			{
				...
				{
		%>
			<option <%SPHttpUtility.WriteNoEncode((field.JumpToNoField == spFieldTmp.InternalName)?"selected":"",this.Page);%> Value=<%SPHttpUtility.WriteHtmlEncode(spFieldTmp.InternalName,this.Page);%>><%SPHttpUtility.WriteHtmlEncode(TruncatedTitle(spFieldTmp.Title),this.Page);%></option>
		<%
				}
			}
%>
...
%>
			<tr>
			<td class="ms-authoringcontrols"><label for="onetidanybranch"><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,SurveyBranchAnyResponse%>" EncodeMethod='HtmlEncode'/></label></td>
			<td class="ms-authoringcontrols">
			<select name="JumpTo" id="onetidanybranch">
				<option value=""><SharePoint:EncodedLiteral runat="server" text="<%$Resources:wss,SurveyBranchNoBranching%>" EncodeMethod='HtmlEncode'/></option>
<%
			for (int i=indexOfCurrentField+1; i<spFields.Count; i++)
			{
				...
				{
		%>
			<option <%SPHttpUtility.WriteNoEncode((spField.JumpToField == spFieldTmp.InternalName)?"selected":"",this.Page);%> Value=<%SPHttpUtility.WriteHtmlEncode(spFieldTmp.InternalName,this.Page);%>><%SPHttpUtility.WriteHtmlEncode(TruncatedTitle(spFieldTmp.Title),this.Page);%></option>
		<%
				}
			}
%>
...
<%
			if (bCanBeDeleted) {
%>
				<input class="ms-ButtonHeightWidth" id="onetidDeleteItem" type="Button" accesskey="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_delete_accesskey%>' EncodeMethod='HtmlEncode'/>" value="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,fldedit_delete%>' EncodeMethod='HtmlEncode'/>" onclick="DoDelete()" />
<%
			}
%>
&#160; &#160; &#160; &#160; &#160; &#160; &#160; &#160; &#160;
		...
		<input class="ms-ButtonHeightWidth" id="onetidSaveItem" accesskey="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,multipages_okbutton_accesskey%>' EncodeMethod='HtmlEncode'/>"  type="button" value="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,multipages_okbutton_text%>' EncodeMethod='HtmlEncode'/>" onclick="SubmitPage(false,this);"/>
		<span id="idSpace" class="ms-SpaceBetButtons"></span>
		<input class="ms-ButtonHeightWidth" id="onetidCancelItem" accesskey="c" type="button" value="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,multipages_cancelbutton_text%>' EncodeMethod='HtmlEncode'/>" onclick="if(typeof(window.frameElement) != 'undefined' && window.frameElement != null) { window.frameElement.cancelPopUp(); return false;} if (window.history.length > 0) { window.history.back(); return false;} else { return false;}" />
</td> </tr> </table> </td></tr>
	<tr>
